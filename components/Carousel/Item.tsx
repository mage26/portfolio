...
const CarouselItem = ({
  ...
  const handleContentScroll = useCallback(() => {
    ...
    const { width: contentWidth, left: contentLeft } =
      contentElementRef.current.getBoundingClientRect();

    const { width: itemWidth, left: itemLeft } =
    ...
    const positionLeftPercentage =
      positionLeft / contentWidth + itemWidthPercentage / 2;
  ...
  useEffect(() => {
    contentElementRef.current?.addEventListener("scroll", handleContentScroll);
    handleContentScroll();
