...
import { forwardRef, TouchEventHandler, useCallback, useEffect, useEffectEvent, useRef } from 'react';
...
const CarouselScrollbarThumb = forwardRef<HTMLDivElement, CarouselScrollbarThumbProps>(({
    ...
    const updateThumbSize = useCallback(() => {
        if (!contentElementRef.current || !scrollbarTrackRef.current || !scrollbarThumbRef.current) { return; }

        const trackWidth = scrollbarTrackRef.current.clientWidth;
        
        const { clientWidth: contentWidth, scrollWidth: contentScrollWidth } = contentElementRef.current;

        const thumbSize = Math.max((contentWidth / contentScrollWidth) * trackWidth, MIN_THUMB_SIZE);

        scrollbarThumbRef.current.style.width = `${thumbSize}px`;
    }, [contentElementRef, scrollbarThumbRef, scrollbarTrackRef]);

    const updateThumbPosition = useCallback(() => {
        if (!contentElementRef.current || !scrollbarTrackRef.current || !scrollbarThumbRef.current || isDragging) { return; }
    ...
    const handleMove = useCallback((clientX: number) => {
        if(!scrollbarThumbRef.current || !scrollbarTrackRef.current || !contentElementRef.current || !thumbOffsetRef.current) { return; }
        ...
        const thumbLeftMax = trackWidth - thumbWidth;
        const clientXRelative = clientX - trackRectangle.left - thumbOffsetRef.current;
    ...
    const onDragEnd = useCallback(() => {
        ...
        const stopPoints = itemsElementsRefs.current.map((itemRef) => itemRef.offsetLeft);

        const currentScrollLeft = contentElementRef.current?.scrollLeft || 0;
        const closestStopPoint = stopPoints.reduce((previous, current) => (Math.abs(current - currentScrollLeft)
            < Math.abs(previous - currentScrollLeft) ? current : previous    
        ));
    ...
    }, [contentElementRef, itemsElementsRefs, onMouseMove, onTouchMove, setIsDragging])
    
    const onDragStart = useCallback((clientX: number) => {
        if(!scrollbarThumbRef.current || !scrollbarTrackRef.current) { return; }
    ...
    }, [onDragEnd, onMouseMove, onTouchMove, scrollbarThumbRef, scrollbarTrackRef, setIsDragging]);
